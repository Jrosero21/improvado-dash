...
const ACCENT = "#5B39F5";
const PLT = { facebook: "#1877F2", google: "#4285F4", tiktok: "#111111" };
...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [ov,pv,pl,ts,ca,tk,gg,fb] = await Promise.all([
        get("overview",{days}), get("overview/prev",{days}),
        get("platforms",{days}), get("timeseries",{days}),
        get("campaigns",{days,limit:10}),
        get("tiktok-funnel",{days}), get("google-quality",{days}),
        get("facebook-engagement",{days}),
      ]);
  ...
  const send = async (text) => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1000, system:buildCtx(),
    ...
    <div style={{ height:"100vh", background:T.bg, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"Inter,sans-serif" }}>
      <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:14, padding:"36px 40px", maxWidth:420, textAlign:"center" }}>
        ...
        <code style={{ display:"block", background:T.bg, borderRadius:8, padding:"12px 16px", fontSize:13, color:ACCENT, marginBottom:20, textAlign:"left", lineHeight:1.8 }}>
          cd dashboard-api<br/>npm install && node server.js
        </code>
            ...
            <Card style={{ flex:"0 0 62%",minWidth:0 }}>
                ...
                <div style={{ display:"flex",gap:14 }}>
                  {(pl||[]).map(p=>(
                    <span key={p.platform} style={{ display:"flex",alignItems:"center",gap:5,fontSize:11,color:T.sub }}>
                      <span style={{ width:16,height:2,background:PLT[p.platform]||ACCENT,display:"inline-block",borderRadius:1 }}/>
                      {p.platform}
                    </span>
                  ))}
                </div>
              ...
              <ResponsiveContainer width="100%" height={170}>
                <LineChart data={pivot} margin={{top:2,right:2,bottom:0,left:0}}>
                  ...
                  {(pl||[]).map(p=>(
                    <Line key={p.platform} type="monotone" dataKey={p.platform+"_spend"} stroke={PLT[p.platform]||ACCENT} strokeWidth={2} dot={false} name={p.platform}/>
                  ))}
                </LineChart>
              </ResponsiveContainer>
            </Card>
              ...
              {(pl||[]).map(p=>{
                ...
                return (
                  <div key={p.platform} style={{ marginBottom:13 }}>
                    ...
                    <div style={{ background:T.bg,borderRadius:3,height:4 }}>
                      <div style={{ background:PLT[p.platform]||ACCENT,height:"100%",borderRadius:3,width:pct+"%" }}/>
                    </div>
                  </div>
                );
              })}
              <div style={{ borderTop:`1px solid ${T.border}`,marginTop:16,paddingTop:14 }}>
                <table style={{ width:"100%",fontSize:12,borderCollapse:"collapse" }}>
                  ...
                  <tbody>
                    {(pl||[]).map(p=>(
                      <tr key={p.platform}>
                        <td style={{ padding:"6px 0",fontWeight:600,color:PLT[p.platform]||ACCENT,textTransform:"capitalize",fontSize:12 }}>{p.platform}</td>
